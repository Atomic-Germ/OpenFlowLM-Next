...
PackOp parse_op(const json& j, const std::string& where) {
    ...
    if (p.op == "std_perm" || p.op == "std_perm_gguf" || p.op == "put" || p.op == "expert_down" || p.op == "conv_transpose" || p.op == "lmhead_q8") {
    ...
    if (p.op == "std_perm" || p.op == "std_perm_gguf") need_all({{"nch", p.nch}, {"in_dim", p.in_dim}});
