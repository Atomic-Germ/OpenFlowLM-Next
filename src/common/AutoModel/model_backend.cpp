...
///       see model_backend.hpp for the renaming and extension notes.
...
/// \brief read OFLM_BACKEND, falling back to the pre-rename FLM_BACKEND
/// \return the override, or empty when neither is set
std::string EnvBackend() {
    return utils::getenv_oflm("OFLM_BACKEND");
}
...
std::string resolve_backend_id(const std::string& family,
    ...
    if (!BackendRegistry::instance().has(family, chosen)) {
        throw std::runtime_error(
            "Backend '" + chosen + "' (from " + chosen_source +
            ") is not compiled into this build of oflm. Model family '" +
            family + "' provides: " +
