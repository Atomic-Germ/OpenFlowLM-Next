...
///       OFLM_USE_HRX / oflm_rt / OFLM_PLATFORM replace the FLM names, with
///       the pre-rename FLM_PLATFORM honoured as a fallback.
...
/// \brief read OFLM_PLATFORM (FLM_PLATFORM fallback), warning once about an unusable value
/// \return the override, or nullopt when unset or unparseable
std::optional<npu_platform> platform_override() {
    const std::string configured = utils::getenv_oflm("OFLM_PLATFORM");
    if (configured.empty()) return std::nullopt;
    const auto parsed = parse_platform(configured);
    if (!parsed) {
        header_print_r("ERROR", "Ignoring unknown OFLM_PLATFORM '"
...
#ifndef OFLM_USE_HRX
int get_platform_columns(const oflm_rt::device& device) {
    const auto platform = nlohmann::json::parse(
        device.get_info<oflm_rt::info::device::platform>());
...
std::optional<int> probe_npu_columns(const oflm_rt::device* device) noexcept {
#ifndef OFLM_USE_HRX
    if (device != nullptr) {
        try {
            ...
            // description must not be fatal for `oflm list` and friends.
        }
    }
#else
    (void)device;
...
npu_platform detect_npu_platform(const oflm_rt::device* device,
    ...
    std::call_once(once, [device]() {
        try {
            if (const auto forced = platform_override()) {
                cached = *forced;
                cached_detail = "forced by OFLM_PLATFORM";
                return;
            }
