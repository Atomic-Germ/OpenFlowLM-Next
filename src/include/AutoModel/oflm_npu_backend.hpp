/// \file oflm_npu_backend.hpp
/// \brief The OpenFlowLM NPU backend, shared by every family that ships one
/// \author OpenFlowLM Team
/// \note Ported from the FastFlowLM fork's flm_npu_backend.hpp (MIT).
///       Every OFLM engine is built the same way -- construct, load the Q4NX
...
/// \brief the default backend for an OpenFlowLM NPU engine
...
class OflmNpuBackend final : public ModelBackend {
public:
    explicit OflmNpuBackend(const BackendContext& context) {
        if (!context.config) {
            throw std::runtime_error("oflm_npu backend needs an LM_Config");
        }
        if (!context.npu) {
            throw std::runtime_error("oflm_npu backend needs an NPU instance");
        }
