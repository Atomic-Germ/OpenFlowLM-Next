// host_var.cpp -*- C++ -*-
// Generic square int16 matmul host for matmul_single_core.py (single core).
// Usage: host_var.exe -x <xclbin> -k MLIR_AIE -i <insts.bin> [--m 64 --k 64 --n 64]
#include <cstdint>
#include <cstring>
#include <fstream>
#include <iostream>
...
int main(int argc, const char *argv[]) {
  int DIM_M = 64, DIM_K = 64, DIM_N = 64;
  if (const char *e = getenv("MM_DIM")) DIM_M = DIM_K = DIM_N = atoi(e);

  cxxopts::Options options("matmul-var-host");
  ...
  std::cout << "mma " << DIM_M << "x" << DIM_K << "x" << DIM_N
            << " max_abs_err=" << max_abs_err << " mismatches=" << errors << "/"
            << DIM_M * DIM_N << "\n";
  std::cout << "C[0:8] =";
  for (int j = 0; j < 8; j++) std::cout << " " << c[j];
  std::cout << "\n";
  if (const char *dump = getenv("DUMPC")) {
    std::ofstream f(dump, std::ios::binary);
    f.write((char *)c, DIM_M * DIM_N * sizeof(int16_t));
  }
  if (!errors) { std::cout << "PASS!\n"; return 0; }
