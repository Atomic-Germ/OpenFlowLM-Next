...
from recipes import dense, qwen36moe as M  # noqa: E402
...
def test_one_rounding_is_enough_because_512_already_tiles_the_chunk():
    """The chunk is 256 columns and a band is 64 rows, so BAND_ROWS * 8 = 512 is
    already a multiple of 256 - no lcm is needed."""
...
def test_padding_does_not_clear_the_moe_core_scratch():
    ...
    # tab_bytes is 2.25 K, so the two tables want 13824 B against the 9216 B the
    # widest single width (the 4096 q projection) reserves.
    assert M.tab_bytes(3072) + M.tab_bytes(3072) > M.tab_bytes(4096)
