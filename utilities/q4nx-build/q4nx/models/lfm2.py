...
class LFM2(__Q4NX_Converter, model_arch=ModelArch.LFM2):
    ...
    def _build_hf_name_map(self):
        hf_to_q4nx = super()._build_hf_name_map()
        hf_to_q4nx["model.embed_tokens.weight"] = "model.token_embd.weight"
        return hf_to_q4nx

    def _convert_hf(self, q4nx_path: str, weights_type: str):
        if weights_type != "language":
            raise ValueError(f"Unsupported weights_type: {weights_type} for LFM2 HF conversion")
        self.q4nx_tensors = {}
