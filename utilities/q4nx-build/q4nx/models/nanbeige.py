...
class Nanbeige(__Q4NX_Converter, model_arch=ModelArch.NANBEIGE):
    ...
    def _convert_hf(self, q4nx_path: str, weights_type: str):
        if weights_type != "language":
            raise ValueError(f"Unsupported weights_type: {weights_type} for Nanbeige HF conversion")
        self.q4nx_tensors = {}
        ...
        if config_path.is_file():
            ...
            with open(config_path) as f:
                cfg = json.load(f)
            rope_dim = cfg.get("rope_dim") or cfg.get("head_dim")
        ...
        for name in sorted(self.weight_map):
            ...
            if ("self_attn.q_proj.weight" in key or "self_attn.k_proj.weight" in key) and rope_dim is not None:
                pp = rope_dim // 2
                w = rearrange(w, '(g p q) c -> (g q p) c', p=pp, q=2).contiguous()
